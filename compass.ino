...
void magcalMPU9250()
{
  ...
  mpu.magbias[0] = (mag_max[0] + mag_min[0])/2; // get average x mag bias in counts
  mpu.magbias[1] = (mag_max[1] + mag_min[1])/2; // get average y mag bias in counts
  mpu.magbias[2] = (mag_max[2] + mag_min[2])/2; // get average z mag bias in counts

  // save the settings in the eeprom, scale stored values by 1/2 to be < 255
  EEPROM.write(eeMagBias, uint8_t(mpu.magbias[0] / 2));;
  EEPROM.write(eeMagBias + 1, uint8_t(mpu.magbias[1] / 2));
  EEPROM.write(eeMagBias + 2, uint8_t(mpu.magbias[2] / 2));
